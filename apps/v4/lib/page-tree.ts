...
// Get current component documentation source from pathname.
export function getCurrentBase(pathname: string): string {
  const baseMatch = pathname.match(
    /\/docs\/components\/(radix|base|aria|npm)\//
  )
